...
class MicroChild(Model):
			...
			for layer_id in range(self.num_layers + 2):  
				...
				if (self.use_aux_heads and
					...
					with tf.variable_scope("aux_head"):
						...
						if (aux_logits.get_shape()[2] - 3) % 5 ==0:
	...
	def connect_controller(self, controller_model):
		if self.fixed_arc is None:
			...
			self.normal_arc_tf = tf.convert_to_tensor(self.normal_arc, dtype=tf.int32)
			self.reduce_arc = fixed_arc[4 * self.num_cells:]
			self.reduce_arc_tf = tf.convert_to_tensor(self.reduce_arc, dtype=tf.int32)
