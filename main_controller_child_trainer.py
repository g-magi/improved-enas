...
DEFINE_integer("num_epochs", 4," = (10+ 20+ 40+ 80)") #original 150
					...
					if (FLAGS.controller_training and
						...
						# addestramento del controller quando il predittore è addestrato
							...
							for ct_step in range(FLAGS.controller_train_steps *
								...
								loss, entropy, lr, gn, val_acc, bl, skip, _ = sess.run(run_ops, feed_dict={self.placeholder_reward: prediction})
								controller_step = sess.run(controller_ops["train_step"])
								if ct_step % FLAGS.log_every == 0:
									curr_time = time.time()
									log_string = "\n"
						...
						for _ in range(10):
							arc, acc = sess.run([
								...
								controller_ops["valid_acc"],
								#controller_ops["inputs_seq"],
							])
							#print("all inputs = ",inputs_seq)
